"""Central registry for all paradise tools.
...
class ToolRegistry:
    ...
    def __init__(self):
        ...
        # Mutations can happen while other threads are reading tool metadata,
        # so keep mutations serialized and readers on stable snapshots.
        self._lock = threading.RLock()
        # Monotonically-increasing generation counter. Bumped on every
        # mutation (register / deregister). External callers (e.g.
        # get_tool_definitions) can memoize against it: a cache entry keyed
        # on the generation is valid for as long as the generation hasn't
        # changed.
    ...
    def register(
        ...
        with self._lock:
            ...
            if existing and existing.toolset != toolset:
                # Reject shadowing — prevent plugins from overwriting
    ...
    def deregister(self, name: str) -> None:
        ...
        with self._lock:
            ...
            if entry is None:
                return
            # Drop the toolset check if this was the last tool in
            # that toolset.
